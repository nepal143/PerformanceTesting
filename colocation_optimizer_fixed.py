#!/usr/bin/env python3
"""
🏢 COLOCATION OPTIMIZER FOR BINANCE & OKX INSTITUTIONAL (FIXED)
Simulates and optimizes co-location trading scenarios for ultra-low latency
Compatible with current websockets library
"""
...
class ColocationOptimizer:
    ...
    async def test_binance_colocation_simulation(self, duration=10):
        ...
        try:
            # Optimized connection settings for colocation
            async with websockets.connect(
                url,
                ping_interval=None,
                ping_timeout=None,
    ...
    async def test_okx_institutional_simulation(self, duration=10):
        ...
        # Use AWS Singapore for better OKX connectivity
        url = "wss://ws.okx.com:8443/ws/v5/public"
        ...
        try:
            ...
            async with websockets.connect(
                ...
                while time.perf_counter() - start_time < duration:
                    try:
                        msg_start = time.perf_counter()
                        msg = await asyncio.wait_for(ws.recv(), timeout=0.005)  # Ultra-fast timeout
                        recv_time = time.perf_counter()
...
async def main():
    ...
    print("🏢 COLOCATION OPTIMIZER FOR BINANCE & OKX INSTITUTIONAL (FIXED)")
    print("=" * 80)
    ...
    if results:
        ...
        for result in binance_results:
            ...
            if 'improvement_percent' in result:
                ...
                print(f"   📊 {connection_type}: {result['avg_latency']:.2f}ms avg ({result['msg_per_sec']:.1f} msg/s)")
        ...
        # ROI Analysis
        print("\n💰 ROI ANALYSIS:")
        if binance_results and okx_results:
            best_binance = min(binance_results, key=lambda x: x.get('min_latency', x.get('avg_latency', 1000)))
            best_okx = min(okx_results, key=lambda x: x.get('min_latency', x.get('avg_latency', 1000)))
        ...
        print("\n💡 PRACTICAL STEPS:")
        print("   • Contact AWS/Alibaba for dedicated hosting quotes")
        print("   • Negotiate with exchanges for institutional access")
        print("   • Test with smaller capital before full deployment")
        print("   • Monitor latency continuously for optimization")
